...
public class HawkeyeWeapons extends JFrame {

	JPanel contentPane;
	JLabel header = new JLabel("Choose Hawkeye's weapons.");
	JLabel main_weapon = new JLabel("Main weapon: Hawkeye's bow");
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					HawkeyeWeapons frame = new HawkeyeWeapons();
	...
	public HawkeyeWeapons() {
		...
		shield.setBounds(142, 122, 141, 23);
	...
	public HawkeyeWeapons(Player p) {
		this();
			...
			public void actionPerformed(ActionEvent e) {
				if (hammer.isSelected()) {
					...
					MainScreen.textArea.append("Hawkeye's sub weapon is set to hammer.\n");
				}
				if (shield.isSelected()) {
					Shield shield = new Shield("shield", 35);
					p.setSubWeapon(shield);
					MainScreen.textArea.append("Hawkeye's sub weapon is set to shield.\n");
				}
				if (gun.isSelected()) {
					Gun gun = new Gun("gun", 4);
					p.setSubWeapon(gun);
					gun.load();
					MainScreen.textArea.append("Hawkeye's sub weapon is set to gun.\n");
				}
				if (staff.isSelected()) {
					Staff staff = new Staff("staff", 20);
					p.setSubWeapon(staff);
					MainScreen.textArea.append("Hawkeye's sub weapon is set to staff.\n");
				}
				dispose();
			}
		});
